 ...
 *
 * Full source's copyright information can be found in the "/full_copyright/old_niobiocash_1_copyright.txt"
*/
  ...
  if (result != 0) {
    if (result == 1) {
      ...
      if (UPNP_AddPortMapping(urls.controlURL, igdData.first.servicetype, portString.str().c_str(),
        portString.str().c_str(), lanAddress, Q1v::CRYPTONOTE_NAME, "TCP", 0, "0") != 0) {
...
namespace Q1v
{
  namespace
  {
    ...
    const command_line::arg_descriptor<std::string> arg_p2p_bind_port      = {"p2p-bind-port", "Port for p2p network protocol", std::to_string(Q1v::P2P_DEFAULT_PORT)};
  ...
  NodeServer::NodeServer(System::Dispatcher& dispatcher, Q1v::CryptoNoteProtocolHandler& payload_handler, Logging::ILogger& log) :
    m_dispatcher(dispatcher),
    m_workingContextGroup(dispatcher),
    m_payload_handler(payload_handler),
    m_allow_local_ip(false),
    m_hide_my_port(false),
    m_network_id(Q1V_NETWORK),
...
#undef INVOKE_HANDLER
  ...
  bool NodeServer::connections_maker()
  {
    ...
    size_t expected_white_connections = (m_config.m_net_config.connections_count * Q1v::P2P_DEFAULT_WHITELIST_CONNECTIONS_PERCENT) / 100;
