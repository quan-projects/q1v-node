...
namespace Q1v
{
  ...
  bool miner::init(const MinerConfig& config) {
    if (!config.extraMessages.empty()) {
      ...
      std::string filebuf;
      if (Common::loadFileToString(m_config_folder_path + "/" + Q1v::parameters::MINER_CONFIG_FILE_NAME, filebuf)) {
    ...
    while(!m_stop)
    {
      ...
      if (!m_stop && check_hash(h, local_diff))
      {
        ...
        if(!m_handler.handle_block_found(b)) {
          ...
          Common::saveStringToFile(m_config_folder_path + "/" + Q1v::parameters::MINER_CONFIG_FILE_NAME, storeToJson(m_config));
        }
      }
